    ...
    <div className="space-y-6">
      ...
      <div className="bg-black text-white p-6 rounded-sm relative overflow-hidden group">
        <div className="relative z-10">
          <h2 className="text-xl font-black uppercase tracking-tight mb-1">{t('admin.hello')}, {user.username}!</h2>
          <p className="text-gray-400 text-xs font-medium">{t('admin.systemStatus')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div>
            <h4 className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-1">{t('admin.teamMembers')}</h4>
        ...
        >
          <div>
            <h4 className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-1">{t('admin.activities24h')}</h4>
        ...
        >
          <div>
            <h4 className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-1">{t('admin.systemUptime')}</h4>
            <p className="text-xl font-black tracking-tight uppercase">99.9%</p>
          </div>
          <div className="w-10 h-10 bg-gray-50 rounded-sm flex items-center justify-center text-black">
            <ShieldCheck size={18} />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 bg-white border border-black/5 rounded-sm">
          <div className="p-4 border-b border-gray-50 flex items-center justify-between">
            <h3 className="text-[10px] font-black uppercase tracking-widest">{t('admin.recentLogs')}</h3>
          </div>
          <div className="p-4 space-y-3">
            {(logs || []).slice(0, 5).map((log, idx) => (
              <div key={idx} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                <div className={`w-6 h-6 rounded-sm flex items-center justify-center flex-shrink-0 text-[10px] ${log.action === 'CREATE' ? 'bg-green-100 text-green-700' :
            ...
            {(logs || []).length === 0 && (
              <div className="p-8 text-center">
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-300 italic">No recent activity</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-gray-50 p-4 border border-black/5 rounded-sm">
            <h4 className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-3">{t('admin.quickActions')}</h4>
